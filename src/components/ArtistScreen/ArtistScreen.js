...
import { getArtistAlbumsById, getMultipleAlbums } from '../../common/client-api-calls'
...
const ArtistScreen = ({artist, yourAlbums, player, addNewScreen, token,}) => {
    ...
    return <>
      ...
      {yourAlbums.map((a, idx) => 
        <p className="menuItem isMenu" 
          key={idx} 
          onClick={(e) => addNewScreen(e,<SongList album={a} player={player} token={token}/>)}>{a.name}</p>)}
      <p className='subHeader' style={{marginTop: '25px'}}>OTHER ALBUMS</p>
