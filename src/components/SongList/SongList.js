...
const SongList = ({album, player, token}) => {
    ...
    useEffect(() => {
        const { tracks: {items}, uri } = album;
        
        setItems(items);
        setAlbumURI(uri);

        const checkForNowPlaying = (state) => {
            const {track_window: {current_track: {id, linked_from}}} = state;
        ...
        return () => player.removeListener('player_state_changed', checkForNowPlaying);
    }, [player, album]);

    return (
        <div className='songList'>{items && items.map((song,idx) => {
            const {track_number, name, id} = song;
            const isPlaying = isThere === idx;
            return <p data-id={id} className={`menuItem${isPlaying ? ' playing' : ''}`} key={idx} onClick={() => playSongFromAlbum(track_number - 1, albumURI, id)}>{name}</p>
        })}</div>
    );
}
