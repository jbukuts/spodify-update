...
import { getCurrentlyPlaying, getAlbumById, getSongLyrics } from '../../common/client-api-calls';
...
const NowPlaying = ({player: p, addScreen, token, createArtistScreen}) => {
    ...
    const goToAlbumClick = () => {
        getCurrentlyPlaying(token).then(r => {
            const {item: {album: {href}}} = r;
            const albumId = href.substring(href.lastIndexOf('/')+1);
            getAlbumById(token, albumId).then(a => {
                const { name } = a;
                addScreen({target: {innerText: name}}, <SongList album={a} player={player} token={token}/>);
            });
        });
    };
    ...
    const goToLyricsClick = () => {
        getSongLyrics(trackName, artistName.name).then(r => {
            const {ok, data} = r;
            if (ok) {
                addScreen({target: {innerText: trackName}}, <LyricsScreen lyricsBody={data.lyrics_body}/>);
            }
        });
    }
    ...
    useEffect(() => {
        ...
        const handleStateChange = (state) => {
            ...
            scrubHandleRef.current.style.left = `${position / (position+timeLeft) * 100}%`;
            scrubLengthRef.current.style.width = `${position / (position+timeLeft) * 100}%`;
            ...
            const mouseUpScrub = (e) => {
                if (mouseDown.current) {
                    player.getCurrentState().then(async (state) => {
                        ...
                        await player.seek(duration * (scrubLengthRef.current.offsetWidth / scrubRef.current.offsetWidth));
                        await player.resume();
    ...
    return (<div className='nowPlaying'>
        ...
        <div>
            <p className='playerTime'>{secondsToMinutes(elapsed)} -{secondsToMinutes(left)}</p>
        </div>
    </div>);
