...
export async function getUsersAlbumsSpotify(accessToken) {
    ...
    try {
        ...
        return fullAlbumList.sort((a,b) => a.name.localeCompare(b.name)).filter(a => a.name !== '');
    } 
    catch(e) {
        console.error(e);
    }
...
export async function playSongByAlbumURI(accessToken, albumURI, trackNumber = 0) {
...
export function getSongLyrics(songName, artistName) {
    ...
    return fetch(`${corsAnywhere}${base}/track.search?q_track=${songName}&q_artist=${artistName}&apikey=${key}&f_has_lyrics=true`, {
        headers: {
            ...apiHeaders,
    ...
    .then(r => r.json())
    .then(r => {
        const { header: {status_code}, body: { track_list }} = r.message;
        if (status_code === 200 && track_list.length > 0) {
            const {track: {track_id }} = track_list[0];
            return fetch(`${corsAnywhere}${base}/track.lyrics.get?track_id=${track_id}&apikey=${key}`, {
                headers: {
                    ...apiHeaders
                }
            })
            .then(r => r.json())
            .then(r => {
                const {header: {status_code}} = r.message;
                if (status_code === 200) {
                    return {
                        ok: true,
...
export function getArtistAlbumsById(accessToken, artistId, limit = 5) {
    try {
        return fetch(`${baseAPIURL}/artists/${artistId}/albums?include_groups=album&limit=${limit}`, {
            headers: {
                ...apiHeaders,
                'Authorization': `Bearer ${accessToken}`
            }
...
export function performSearch(accessToken, queryString, types = 'album', limit=20) {
    try {
        return fetch(`${baseAPIURL}/search?q=${queryString}&type=${types}&limit=${limit}`, {
            headers: {
                ...apiHeaders,
                'Authorization': `Bearer ${accessToken}`
            }
